...
#define HaltNxFault(tf)                                                        \
  tf->Rip = *(ULONG64 *)tf->Rsp;                                               \
  tf->Rsp += 8
...
BOOLEAN ToIgnoreIpxe(size_t ipxeIndex) {
  ...
  if (ipxeVaType == 0xFF) {
    LogInfo(
        "ToIgnoreIpxe: Warning, couldn't fetch pml4 index. (is it usermode?)");
    return TRUE;
  }

  using namespace MiSystemVaType;

  INT32 toIgnoreList[] = {MiVaProcessSpace, MiVaDriverImages, MiVaPagedPool};
...
NTSTATUS NTAPI wsbp::Barricade::HkMmAccessFault(_In_ ULONG FaultCode,
                                                _In_ PVOID Address,
  ...
  if (_ReturnAddress() == gl::RtVar::FaultingAddrPtr) {
    ...
    // The Fault is not occured in user mode, nor it is caused by illegal write
    // access.
...
BOOLEAN wsbp::Barricade::CustomNxFaultHandler(void *faultAddress,
  ...
  if (curIrql >= DISPATCH_LEVEL) {
    ...
    if (!memcmp(instBytes, "\x2E\x48\x31", 3) && // The instruction is same
        !IsCanonicalAddress(
            (PVOID)trapFrame->Rdx) && // The key (to decrypt), it is not normal
                                      // DeferredContext
        trapFrame->Rcx == trapFrame->Rip) {

      LogInfo("NxF: CmpAppendDllSection call detected (%llX), halting",
              trapFrame->Rip);
    ...
    // KiDpcDispatch bypass
             ...
             !IsCanonicalAddress(
                 (PVOID)trapFrame
                     ->Rdx) && // Also same, it is not normal DeferredContext
             trapFrame->Rip - 0x60 <
                 trapFrame->Rcx && // I do not want to modify code every kernel
                                   // update. I saw it [rcx+48h] but it changes
                                   // frequently.
             trapFrame->Rip + 0x60 > trapFrame->Rcx) { // But this works. so.

      LogInfo("NxF: KiDpcDispatch call detected (%llX), halting",
              trapFrame->Rip);
      HaltNxFault(trapFrame);
    ...
    // Detect KiTimerDispatch by searching pushfq instruction & after sub rsp,
    // XXX instruction, which is very(?) rare.
    for (size_t i = 0; i < 0x20; i++) {
      if (instBytes[i] == 0x48 && instBytes[i + 1] == 0x9C) { // pushfq
        for (size_t j = i; j < i + 0x20; j++) {
          if (instBytes[j] == 0x48 && instBytes[j + 1] == 0x83) {

            LogInfo("NxF: KiTimerDispatch call detected (%llX), halting",
                    trapFrame->Rip);
            HaltNxFault(trapFrame);
    ...
    // Currently there are no PG routine that requires more than DISPATCH_LEVEL
    // irql.
    if (curIrql > DISPATCH_LEVEL) {
  ...
  for (size_t step = 0; step < 0x20; step++) {
    ...
    if (value != KeDelayExecutionThread && value != KeWaitForMultipleObjects &&
...
BOOLEAN wsbp::Barricade::FlipPteNxBitIdPc(KDPC *Dpc, PVOID DeferredContext,
                                          PVOID SystemArgument1,
                                          PVOID SystemArgument2) {
  UNREFERENCED_PARAMETER(Dpc);
  UNREFERENCED_PARAMETER(DeferredContext);

  auto LambdaRecursePteToFlip = [](auto &&self, PVOID virtualAddress,
                                   size_t remainingRecurse) {
    UINT64 *ptEntryPtr =
        GetPageTableEntryPointer(virtualAddress, remainingRecurse);
    ...
    if (remainingRecurse > 1) { // not PTE
      ...
      if (!((*ptEntryPtr >> 7) & 0x1)) { // not large page
        ...
        for (size_t ipte = 0; ipte < 512; ipte++) {
          self(self,
               (PVOID)((uintptr_t)virtualAddress |
                       (ipte << (12 + 9 * (remainingRecurse - 2)))),
               remainingRecurse - 1);
        }
        return;
      }
    ...
    if (IsCollapsing(
            (uintptr_t)virtualAddress, (uintptr_t)virtualAddress + pageSize - 1,
            gl::RtVar::Self::SelfBase,
            gl::RtVar::Self::SelfBase + gl::RtVar::Self::SelfSize - 1)) {
      LogVerbose("LambdaRecursePteToFlip: Virtual Address %p is in our driver, "
                 "skipping..",
                 virtualAddress);
      return;
    }
  ...
  // Since we have same PTE base for every core, we can multithread it to
  // improve performance.
  ...
  LogVerbose("FlipPteNxBitIdPc: CPU %llu doing [%llu, %llu)", procNum,
             rangeStart, rangeEnd);

  for (size_t ipxe = rangeStart; ipxe < rangeEnd;
       ipxe++) { // Iterate PML4s to initiate the recursive routine.
    if (ToIgnoreIpxe(ipxe)) {
      continue;
    }

    LambdaRecursePteToFlip(LambdaRecursePteToFlip,
                           MakeCanonicalAddress((PVOID)(ipxe << 39)), 4);
  }
...
BOOLEAN wsbp::Barricade::InjectCustomInterruptHandler() {
  ...
  if (!Hook::HookTrampoline(gl::RtVar::MmAccessFaultPtr, HkMmAccessFault,
                            TrampolineArea, gl::MmAccessFaultInstSize)) {
    LogError("InjectCustomInterruptHandler: Couldn't initiate hook for "
             "MmAccessFault");
    return FALSE;
  }
...
NTSTATUS(NTAPI *wsbp::Barricade::OrigMmAccessFault)(_In_ ULONG, _In_ PVOID,
                                                    _In_ KPROCESSOR_MODE,
                                                    _In_ PVOID) =
    (NTSTATUS(NTAPI *)(_In_ ULONG, _In_ PVOID, _In_ KPROCESSOR_MODE,
                       _In_ PVOID))(TrampolineArea);
